...
function get_list_filename() {
    exec("find /root/swrveexternal-alegrium/ -path *app-1377*.log.gz", $output); // find all log.gz
    asort($output);
...
foreach ($list_filename as $filename) {
    ...
    $status = false;
    $document = $db->swrvelog->findOne(['filename' => $filename, 'app' => '1337']);
    ...
    if ($status == false) {
        ...
        exec("mongoimport --host $db_host --db $db_name --collection swrveexternal_1377 --file $filelog --username $db_user --password $db_pass");
        exec("rm $filelog");
        $db->swrvelog->insertOne(array('filename' => $filename, 'app' => '1337'));
    }
}